  ...
  return (
    <section className="bg-[#E5376B] py-[60px] md:py-[125px]">
      <div className="flex flex-col max-w-[1407px] mx-auto text-center px-4 gap-4 ">
        <h4 className="uppercase text-white text-sm md:text-2xl font-medium leading-[22px] tracking-[1.28px] font-poppins">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-[78px] sm:mt-[35px] max-w-full sm:max-w-[950px] mx-auto lg:max-w-[1050px]">
          {markets.map((market) => (
            <div
              key={market.id}
              className="bg-white rounded-[32px] shadow-lg p-4 lg:p-10 flex flex-col items-center md:max-w-[485px]"
              style={{height:"475px"}}
            >
              <img
                src={market.image}
                alt={market.title}
                className="rounded-[32px] w-[420px] h-[270px] object-cover md:p-0"
              />
              <h3 className="bg-[#A4C428] font-mulish text-white font-medium text-sm md:text-lg py-[10px] md:px-[10px] w-full mt-[30px] leading-6 whitespace-pre">
                {market.title}
              </h3>
              <a
                href={market.link}
